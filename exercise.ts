...
const getAmountPublishedInFantasy = (writers:Writer[], books: Book[], givenWriter:string ) => {
  let id = writers.filter(writer => writer.name === givenWriter)[0].id
  if (!id) return 0
  return books.filter(book => book.writerId === id && book.type === "fantasy").reduce((sum,b)=>sum + b.numberOfWords,0)
